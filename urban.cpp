...
//	spatialinterplore(i, years*16, irow, icol, 5, phyndmi);
    ...
    /*
    ofname=dirc+"/greenness.img";
    ohdrfname=dirc+"/greenness.hdr";
    fout.open(ofname.c_str(), ios::binary);
    fout.write((char*) gre, tsbands*pnum*sizeof(float));
    fout.close();
    obandnames.clear();
    sepbandnames(tsbandnames, tsbands, obandnames);
    writeenviheader(ohdrfname, "Tasselled cap transformation - greenness", ehd.samples, ehd.lines, tsbands, 4, ehd.interleave, ehd.xstart, ehd.ystart, ehd.map_info,  ehd.wavelength_units, obandnames, ehd.projection_info, ehd.coordinate_system_string);

    ofname=dirc+"/wetness.img";
    ohdrfname=dirc+"/wetness.hdr";
    fout.open(ofname.c_str(), ios::binary);
    fout.write((char*) wet, tsbands*pnum*sizeof(float));
    fout.close();
    obandnames.clear();
    sepbandnames(tsbandnames, tsbands, obandnames);
    writeenviheader(ohdrfname, "Tasselled cap transformation - wetness", ehd.samples, ehd.lines, tsbands, 4, ehd.interleave, ehd.xstart, ehd.ystart, ehd.map_info,  ehd.wavelength_units, obandnames, ehd.projection_info, ehd.coordinate_system_string);
